...
class DocumentPayrollHelper
{
    ...
    private function getEndpointDataApi(DocumentPayroll $document, $inputs)
    {
        if($document->is_payroll_adjust_note)
        {
            ...
            $url = ($this->company->payroll_type_environment_id == 2 && $this->company->test_set_id_payroll != 'no_test_set_id') ? "ubl2.1/payroll-adjust-note/{$this->company->test_set_id_payroll}" : "ubl2.1/payroll-adjust-note";
        }
        else
        {
            $params = $this->getParamsForApi($document, $inputs);
            $url = ($this->company->payroll_type_environment_id == 2 && $this->company->test_set_id_payroll != 'no_test_set_id') ? "ubl2.1/payroll/{$this->company->test_set_id_payroll}" : "ubl2.1/payroll";
        }
